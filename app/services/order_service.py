...
from app.crud.order import create_order_record,get_orders_by_uid
...
def create_order(db:Session,u_id:int,p_name:str,quantity:int):
    ...
    if not inven or inven.availbale_stock<quantity:
        raise HTTPException(status_code=400,detail="库存不足")

    order_amount=product.price*quantity
    order=create_order_record(db,u_id,product.id,quantity,order_amount)
    update_inventory(db,inven.id,available_stock=inven.availbale_stock-quantity,locked_stock=inven.locked_stock+quantity)
    return order

def get_user_order(db:Session,u_id:int):
    orders=get_orders_by_uid(db,u_id)
    return orders

def get_order_by_id(db:Session,id:int):
    return order.get_order_by_id(db,id)
